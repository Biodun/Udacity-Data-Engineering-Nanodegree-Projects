...
def process_song_file(cur, filepath):
    ...
    try:
        ...
        print(
            f"ERROR!! The following error occurred while trying to insert song data into the database: {e}"
        )
    ...
    try:
        ...
        print(
            f"ERROR!! The following error occurred while trying to insert artist data into the database: {e}\n{artist_data}"
        )
...
def process_log_file(cur, filepath):
    ...
    time_data = [
        (f"{ts}", ts.hour, ts.day, ts.weekofyear, ts.month, ts.year, ts.weekday())
        for ts in df["ts"]
    ]
    ...
    for i, row in time_df.iterrows():
        try:
            ...
            print(
                f"ERROR!! The following error occurred while inserting time log data into the time table: {e}"
            )
    ...
    for i, row in user_df.iterrows():
        try:
            ...
            print(
                f"ERROR!! The following error occurred while inserting user data into the users table: {e}\n\n"
            )
            raise ValueError
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            print(
                f"ERROR! The following error occurred while retrieving song data for {row.userId} {row.song}, {row.artist}, {row.length}: {e}"
            )
...
def process_data(cur, conn, filepath, func):
    ...
    ----------
    cur : Postgres cursor    
    conn : Postgres connector object    
    filepath : str
        data filepath
    func : Python function
        Function that will be applied to the code
